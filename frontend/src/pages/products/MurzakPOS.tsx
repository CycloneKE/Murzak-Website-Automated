    ...
    <main className="text-white overflow-x-hidden">
      ...
      <section className="relative pt-20 lg:pt-28 pb-16 overflow-hidden">
        ...
        <div className="max-w-[1100px] mx-auto px-6 sm:px-10 lg:px-16">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-murzak-cyan/10 border border-murzak-cyan/20 mb-6">
            <span className="text-[10px] font-black uppercase tracking-widest text-murzak-cyan">Murzak POS & Inventory</span>
          </div>
      ...
      <Section className="border-t border-white/5">
        <div className="max-w-2xl mb-12">
          <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-murzak-cyan mb-3">Core Features</p>
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-[900] tracking-tight">Everything you need to run your shop.</h2>
        </div>
        <div className="grid sm:grid-cols-2 gap-5">
          {features.map((f, i) => (
            <div key={i} className="rounded-3xl border border-white/10 bg-murzak-navy/80 backdrop-blur-md p-7">
            ...
            <ul className="space-y-3">
              {['Barcode scanning support', 'Quick-add categories', 'Instant M-Pesa STK push', 'Custom receipt printing'].map(item => (
                <li key={item} className="flex items-center gap-3 text-sm font-bold text-white">
                  <Check size={16} className="text-murzak-cyan" /> {item}
                </li>
              ))}
            </ul>
          </div>
      ...
      <Section className="border-t border-white/5">
         <div className="text-center max-w-2xl mx-auto mb-12">
            <h2 className="text-3xl font-[900] tracking-tight text-white mb-4">Built for Kenyan Retail</h2>
            <p className="text-slate-400 font-medium">Whether you have one shop or fifty branches, Murzak POS scales with you.</p>
         </div>
         <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {['Supermarkets', 'Pharmacies', 'Hardware Stores', 'Boutiques & Apparel'].map(item => (
              <div key={item} className="p-6 rounded-2xl bg-white/5 border border-white/10 text-center font-bold text-sm text-white">
                {item}
              </div>
            ))}
         </div>
      ...
      <section className="relative py-24 overflow-hidden">
        ...
        <div className="max-w-2xl mx-auto px-6 text-center">
          <h2 className="text-3xl sm:text-4xl font-[900] tracking-tight text-white mb-6">Stop guessing your stock.</h2>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button variant="onDark" onClick={() => onNavigate('/pricing?configure=biz-pos-inventory')}>
