...
const TYPE_COLORS = {
  ...
  system: "bg-murzak-cyan/10 text-murzak-cyan border-murzak-cyan/20",
...
export default function ActivityTimeline({ events }: ActivityTimelineProps) {
  return (
    <div className="relative pl-6 space-y-8 before:absolute before:inset-y-0 before:left-[11px] before:w-px before:bg-gradient-to-b before:from-murzak-cyan/50 before:to-transparent">
      {events.map((event, index) => (
        <div key={event.id} className="relative group animate-fade-in" style={{ animationDelay: `${index * 100}ms` }}>
          <div className={`absolute -left-6 top-1 w-6 h-6 rounded-full border flex items-center justify-center bg-murzak-navy ${TYPE_COLORS[event.type]} shadow-[0_0_10px_currentColor] transition-transform group-hover:scale-110`}>
          ...
          <div className="glass-panel p-4 rounded-2xl hover:border-white/20 transition-all cursor-default">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h4 className="text-[12px] font-black text-white">{event.title}</h4>
                <p className="text-[11px] font-medium text-slate-400 mt-1 leading-relaxed">{event.description}</p>
              </div>
